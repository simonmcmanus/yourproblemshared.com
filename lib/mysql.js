...
var q = 'select email.*, company.name as companyName, company.url as companyUrl  from email, company, domain where email.id="'+params.id+'" and email.company=domain.domain AND company.id=domain.companyId';
	sql.query(q, params, function(data, params) {
		if(data[0] && data[0].messageId) { // lets look for follow up message and send them both in one request.
			...
			var q2 = 'select email.*, company.name as companyName, company.url as companyUrl  from email, company, domain where email.id="'+params.id+'"  OR referenceId LIKE "%'+data[0].messageId+'%" and email.company=domain.domain AND company.id=domain.companyId';
...
exports.company = function(params, callback) {
	...
	var q = 'SELECT email.*, company.name AS companyName, company.url AS companyUrl, company.logo AS companyLogo from email, company, domain  WHERE  isFirst=1  AND company.url="'+params.company+'" AND domain.companyId=company.id and email.company=domain.domain group by subject ORDER BY email.id';
