...
exports.saveEmail = function(params, callback) {
	...
	console.log('yps:emailsByRef:'+params.referenceId, id);
	client.multi([
		['lpush', 'yps:emails', id],
		['lpush', 'yps:emailsByRef:'+params.referenceId, id],
		['hmset', id, 
			'id', params.id,
...
exports.getEmail = function(params, callback) {
	...
	client.hgetall('yps:email:' + params.id, function(error, data) {
		console.log('yps:emailsByRef:'+data['messageId']);
		client.lrange('yps:emailsByRef:'+data['messageId'], 0, -1, function(error, emailIds) {
			var multi = [];
			...
			console.log('', multi);
			client.multi(multi).exec(function (err, emails) {
				console.log('got TO HERE: ', emails);
